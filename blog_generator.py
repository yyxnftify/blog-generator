...
## ★最重要：構成ルール
1. **H2見出しは最大5個まで**（H3は使わない。見出しが多すぎると中身が薄くなる）
2. 各H2の下には十分な本文を書くため、見出しは厳選する
...
6. FAQは見出しとしてではなく、まとめの中に2〜3個含める程度でOK
7. 参考サイト: https://jetb.co.jp のブログ記事のような、読みやすくて中身の濃い構成を目指す"""

    user_prompt = f"""## ターゲットキーワード
{keyword}

## 競合サイトの見出し構造（参考。ただし見出しの数は5個以下に絞ること）
{existing_headings}
...
※重要: h3_listは空配列[]にしてください。H2見出しは必ず5個以下。"""
...
## ★最重要：記事構成ルール
1. **H2見出しは最大5個まで**。H3は基本的に使わない（使うなら1〜2個まで）
2. **各H2セクションの本文は最低5〜8段落（400〜800文字）**書くこと。2〜3行で終わるのはNG
3. 見出しの数を減らす代わりに、各セクションの中身を濃く・深くする
4. 冒頭にリード文（200〜400文字）を入れる。読者の興味を引く書き出し
5. 最後に短い「まとめ」を入れる（長いまとめは不要）

## ★最重要：文体ルール（これがこの記事の命）
1. **AI臭を完全排除する**:
   - 「〜と言えるでしょう」「〜ではないでしょうか」「〜について解説します」のような定型フレーズ禁止
...
2. **人間味のある自然な文体**:
   ...
   - 「実は〜」「ここがポイントで〜」「正直なところ〜」など、リアリティのある表現
   - 体験談・実感を交えた表現（「やってみると分かるのですが〜」）
   - 適度にカジュアル、でも信頼感のある文体

3. **読者への呼びかけ・訴求**:
   - 時々、読者に問いかける表現を入れる（「〇〇って知っていますか？」）
   - 読者の悩みに共感する表現（「こんな経験ありませんか？」）
   - 「へぇ、そうなんだ！」と思わせる意外な事実や数字を盛り込む

4. **著者（ペルソナ）の感想を時々入れる**:
   - 「個人的には〜がおすすめです」「実際に試してみて一番よかったのは〜」
   - 多用せず、記事全体で2〜3箇所にさりげなく入れる
   - 専門家としての実感や、現場の声として

5. **キャッチーな見出し**:
   - 見出しは「本文のまとめ」や「読者への呼びかけ」にする
   - 良い例: 「フィンガーライム、知っていますか？」「プロが教える失敗しない育て方」
   - 悪い例: 「フィンガーライムとは」「栽培方法」（退屈な見出しは禁止）

## ★SEOライティングの技術ルール
1. **1文は40〜60文字以内**を目安に（長文は分割）
2. **段落は3〜5文で改行**する（読みやすさ重視）
3. **見出し（H2）にキーワードを自然に含める**
4. **箇条書き・番号リスト**を効果的に使う（ただし多用しすぎない）
5. **太字（<strong>）**で重要ポイントを強調する
6. **画像挿入ポイント**を<!-- 画像: 説明 -->で示す
...
## 記事構成（この見出し構造に従って執筆。H2は最大5個。各セクションは最低5〜8段落書くこと）
{outline_text}
...
## ★執筆の最終チェックリスト
- H2見出しは5個以下か？ → 多い場合は統合する
- 各セクションの本文は5〜8段落（400〜800文字）以上あるか？ → 2〜3行で終わっていないか確認
- 見出しはキャッチーか？ → 「〇〇とは」のような退屈な見出しになっていないか
- 読者への呼びかけが入っているか？ → 「知っていますか？」「こんな経験ありませんか？」
- 著者の感想が1〜2箇所入っているか？ → 「個人的には〜」「実際に試してみると〜」
- AI臭い表現はないか？ → 「〜と言えるでしょう」等を使っていないか
